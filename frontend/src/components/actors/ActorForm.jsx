...
const ActorForm = ({ onActorAdded, existingActors = [], editingActor, onEditComplete }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEditing) {
      const isDuplicate = existingActors.some(actor => 
        actor.name.toLowerCase() === trimmedName.toLowerCase()
      );
      if (isDuplicate) {
    ...
    try {
      ...
      console.error(`Error ${isEditing ? 'updating' : 'creating'} actor:`, error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form className="glass-form" onSubmit={handleSubmit}>
      ...
      >
        {loading ? (isEditing ? 'Updating...' : 'Adding...') : (isEditing ? 'Update Actor' : 'Add Actor')}
      </button>
    </form>
  );
};
...
ActorForm.propTypes = {
  ...
  existingActors: PropTypes.arrayOf(PropTypes.shape({
    id: PropTypes.number,
